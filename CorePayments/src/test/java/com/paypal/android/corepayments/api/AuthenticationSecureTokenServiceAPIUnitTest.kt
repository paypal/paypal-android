...
class AuthenticationSecureTokenServiceAPIUnitTest {
    ...
    @Test
    fun `getLowScopedAccessToken() makes correct API request with proper headers and body`() =
        runTest {
        // Given
        ...
        // When
            val result = sut.getLowScopedAccessToken()
        ...
        assertTrue(result is GetClientTokenResult.Success)
        assertEquals("test-token", (result as GetClientTokenResult.Success).token)
    }

    @Test
    fun `getLowScopedAccessToken() returns access token from successful response`() = runTest {
        // Given
        ...
        // Then
        assertTrue(result is GetClientTokenResult.Success)
        assertEquals(expectedToken, (result as GetClientTokenResult.Success).token)
    }

    @Test
    fun `getLowScopedAccessToken() returns failure when response is not successful`() = runTest {
        // Given
        ...
        // Then
        assertTrue(result is GetClientTokenResult.Failure)
    }

    @Test
    fun `getLowScopedAccessToken() returns failure when response body is null`() = runTest {
        // Given
        val successResponse = HttpResponse(
            status = 200,
            body = null
    ...
    @Test
    fun `getLowScopedAccessToken() returns failure when response is not valid JSON`() = runTest {
        // Given
        val successResponse = HttpResponse(
            status = 200,
    ...
    @Test
    fun `getLowScopedAccessToken() returns failure when access_token field is missing from successful response`() =
        runTest {
        // Given
        val successResponse = HttpResponse(
            status = 200,
        ...
        coEvery { restClient.send(any()) } returns successResponse
            ...
            // Then
            assertTrue(result is GetClientTokenResult.Failure)
            val error = (result as GetClientTokenResult.Failure).error
            assertTrue(error.errorDescription.contains("Missing access_token in response"))
    }

    @Test
    fun `getLowScopedAccessToken() handles empty response body with proper error message`() =
        runTest {
        // Given
        val errorResponse = HttpResponse(
            status = 500,
        ...
        // Then
        assertTrue(result is GetClientTokenResult.Failure)
        val errorMessage = (result as GetClientTokenResult.Failure).error.errorDescription
        // The error description will be the empty body from the HTTP response
        assertEquals("", errorMessage)
    }

    @Test
    fun `getLowScopedAccessToken() includes error message in exception when available`() = runTest {
        // Given
        ...
        // Then
        assertTrue(result is GetClientTokenResult.Failure)
        val description = (result as GetClientTokenResult.Failure).error.errorDescription
        // The error description will be the error message from the HTTP response body
        assertEquals(errorMessage, description)
    }

    @Test
    fun `getLowScopedAccessToken() creates proper Basic Auth header from client ID`() = runTest {
        // Given
        ...
        // When
        sutWithCustomConfig.getLowScopedAccessToken()
