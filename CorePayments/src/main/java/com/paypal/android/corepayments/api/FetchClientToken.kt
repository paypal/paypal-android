...
public class FetchClientToken(
    private val coreConfig: CoreConfig,
    private val restClient: RestClient = RestClient(configuration = coreConfig),
) {
    suspend operator fun invoke(): String {
        val body = "grant_type=client_credentials&response_type=token"
        ...
        if (httpResponse.isSuccessful) {
            ...
            throw APIClientError.payPalCheckoutError("Error fetching client token: ${httpResponse.status}")
        }
    }
}
