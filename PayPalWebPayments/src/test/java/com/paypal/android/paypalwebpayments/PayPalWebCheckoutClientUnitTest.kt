...
import com.paypal.android.corepayments.api.GetClientTokenResult
...
class PayPalWebCheckoutClientUnitTest {
    ...
    fun beforeEach() {
        ...
        coEvery { authenticationSecureTokenServiceAPI.getLowScopedAccessToken() } returns
                GetClientTokenResult.Success("fake-access-token")
    }
    ...
    fun `start() fetches client token and launches PayPal web checkout`() = runTest {
        ...
        coVerify(exactly = 1) { authenticationSecureTokenServiceAPI.getLowScopedAccessToken() }
    ...
    fun `start() returns failure when FetchClientToken fails`() = runTest {
        ...
        coEvery { authenticationSecureTokenServiceAPI.getLowScopedAccessToken() } returns
                GetClientTokenResult.Failure(tokenError)
