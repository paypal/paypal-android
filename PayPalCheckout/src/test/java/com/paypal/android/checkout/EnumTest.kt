...
class EnumTest {
    ...
    fun `UserAction has a values in payPalCheckout and native`() {
        UserAction.values().forEach { userAction ->
            expectThat(userAction.asNativeCheckout).isEqualTo(
                com.paypal.checkout.createorder.UserAction.valueOf(
                    userAction.name
                )
            )
        }
        com.paypal.checkout.createorder.UserAction.values().forEach { nativeUserAction ->
            expectThat(nativeUserAction.asPaypalCheckout).isEqualTo(
                UserAction.valueOf(
                    nativeUserAction.name
                )
            )
        }
    }
    ...
    fun `OrderIntent has a values in payPalCheckout and native`() {
        OrderIntent.values().forEach { orderIntent ->
            expectThat(orderIntent.asNativeCheckout).isEqualTo(
                com.paypal.checkout.createorder.OrderIntent.valueOf(
                    orderIntent.name
                )
            )
        }
        com.paypal.checkout.createorder.OrderIntent.values().forEach { nativeOrderIntent ->
            expectThat(nativeOrderIntent.asPaypalCheckout).isEqualTo(
                OrderIntent.valueOf(
                    nativeOrderIntent.name
                )
            )
        }
    }
    ...
    fun `PaymentButtonIntent has a values in payPalCheckout and native`() {
        PaymentButtonIntent.values().forEach { paymentButtonIntent ->
            expectThat(paymentButtonIntent.asNativeCheckout).isEqualTo(
                com.paypal.checkout.config.PaymentButtonIntent.valueOf(
                    paymentButtonIntent.name
                )
            )
        }
        com.paypal.checkout.config.PaymentButtonIntent.values()
            .forEach { nativePaymentButtonIntent ->
                expectThat(nativePaymentButtonIntent.asPaypalCheckout).isEqualTo(
                    PaymentButtonIntent.valueOf(
                        nativePaymentButtonIntent.name
                    )
                )
            }
    }
    ...
    fun `ShippingType has a values in payPalCheckout and native`() {
        ShippingType.values().forEach { shippingType ->
            expectThat(shippingType.asNativeCheckout).isEqualTo(
                com.paypal.checkout.createorder.ShippingType.valueOf(
                    shippingType.name
                )
            )
        }
        com.paypal.checkout.createorder.ShippingType.values().forEach { nativeShippingType ->
            expectThat(nativeShippingType.asPaypalCheckout).isEqualTo(
                ShippingType.valueOf(
                    nativeShippingType.name
                )
            )
        }
    }
    ...
    fun `ShippingChangeType has a values in payPalCheckout and native`() {
        ShippingChangeType.values().forEach { shippingChangeType ->
            expectThat(shippingChangeType.asNativeCheckout).isEqualTo(
                com.paypal.checkout.shipping.ShippingChangeType.valueOf(
                    shippingChangeType.name
                )
            )
        }
        com.paypal.checkout.shipping.ShippingChangeType.values()
            .forEach { nativeShippingChangeType ->
                expectThat(nativeShippingChangeType.asPaypalCheckout).isEqualTo(
                    ShippingChangeType.valueOf(
                        nativeShippingChangeType.name
                    )
                )
            }
    }
