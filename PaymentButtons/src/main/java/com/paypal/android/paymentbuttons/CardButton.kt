 ...
 *
 */
open class CardButton @JvmOverloads constructor(
    context: Context,
    attributeSet: AttributeSet? = null,
    defStyleAttr: Int = 0
) : PaymentButton<CardButtonColor>(context, attributeSet, defStyleAttr) {

    /**
     * Updates the color of the Payment Button with the provided [CardButtonColor].
     *
     ...
     */
    override var color: PaymentButtonColor = CardButtonColor.BLACK
        set(value) {
            field = value
    ...
    /**
     * Updates the label for Payment Button with the provided [CardButtonLabel].
     *
     ...
     */
    open var label: CardButtonLabel = CardButtonLabel.CHECKOUT
        set(value) {
            if (value != CardButtonLabel.PAY_LATER) {
                field = value
                updateLabel(field)
            }
        }

    override val wordmarkDarkLuminanceResId: Int = R.drawable.card_white

    override val wordmarkLightLuminanceResId: Int = R.drawable.card_black
    ...
    private fun updateColorFrom(typedArray: TypedArray) {
        val paypalColorAttribute = typedArray.getInt(
            ...
            CardButtonColor.GOLD.value
        )
        color = CardButtonColor(paypalColorAttribute)
    }
    ...
    private fun updateLabelFrom(typedArray: TypedArray) {
        ...
        label = CardButtonLabel(paypalLabelAttribute)
    }

    protected fun updateLabel(updatedLabel: CardButtonLabel) {
        // simulate payment label at the end
        prefixTextVisibility = View.GONE
        suffixTextVisibility = View.VISIBLE
        suffixText = "Pay with Card"
    }
}
...
enum class CardButtonColor(
    ...
    companion object {
        /**
         * Given an [attributeIndex] this will provide the correct [CardButtonColor]. If an
         ...
         */
        operator fun invoke(attributeIndex: Int): CardButtonColor {
            return when (attributeIndex) {
                ...
                BLACK.value -> BLACK
                SILVER.value -> SILVER
                else -> throw createFormattedIllegalArgumentException("CardButtonColor", values().size)
            }
        }
    }
}
...
enum class CardButtonLabel(
    ...
    companion object {
        /**
         * Given an [attributeIndex] this will provide the correct [CardButtonLabel].
         * If an invalid [attributeIndex] is provided then it will throw an [IllegalArgumentException].
         *
         * @throws [IllegalArgumentException] when an invalid index is provided.
         */
        operator fun invoke(attributeIndex: Int): CardButtonLabel {
            return when (attributeIndex) {
                PAYPAL.value -> PAYPAL
