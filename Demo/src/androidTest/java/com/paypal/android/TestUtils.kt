...
/**
 * Sets up app links for the specified package
 * Enables app links and sets user selection to allow handling of supported URLs
 *
 * @param appPackageName The package name of the app to configure (without .test suffix)
 */
fun setupAppLinks(appPackageName: String) {
    val tag = "AppLinksUtils"
    val instrumentation = InstrumentationRegistry.getInstrumentation()

    Log.d(tag, "🔧 Setting up app links for package: $appPackageName")

    // Set app links
    val setAppLinksResult = executeShellCommandWithOutput(
        instrumentation,
        "pm set-app-links --package $appPackageName 1 all"
    )
    Log.d(tag, "pm set-app-links result: $setAppLinksResult")

    // Set user selection
    val setUserSelectionResult = executeShellCommandWithOutput(
        instrumentation,
        "pm set-app-links-user-selection --package $appPackageName true all"
    )
    Log.d(tag, "pm set-app-links-user-selection result: $setUserSelectionResult")
}
